...
#create plot
plot.ts(cbind(rBTC, VaR_rBTC, t_VCoVaR, t_CoVaR_4, t_MCoVaR), plot.type = "single", 
        col = c("blue3", "red3", rgb(0, 0.675, 0), "darkgreen", "skyblue4"),
        main = paste("VCoVaR, SCoVaR and MCoVaR of BTC"), 
        ylim = c(-3.15, 0.225), ylab = "log-returns", xaxt = "n")
axis(1, at = timeAT, labels = timeLAB)
...
#VCoVaR condition
text(coord, at[1], expression(symbol("\044")~i:~ X[i]<="VaR("~X[i]~"):"), pos = pos, cex = 1)
points(V_exc, rep(at[2], length(V_exc)), pch = 4)

#VCoVaR exceedences
text(coord, at[3], expression(BTC<=VCoVaR ~"|"~symbol("\044")~i:~ X[i]<="VaR("~X[i]~"):"), pos = pos, cex = 1, col = rgb(0, 0.675, 0))
points(t_CoVaR_V_exc[[3]], rep(at[4], t_CoVaR_V_exc[[2]]), pch = 4, col = rgb(0, 0.675, 0))

#SCoVaR condition
text(coord, at[5], expression(sum(X[i])<="VaR("~sum(X[i])~"):"), pos = pos, cex = 1)
points(VaR_Sys_exc, rep(at[6], length(VaR_Sys_exc)), pch = 4)

#SCoVaR exceedences
text(coord, at[7], expression(BTC<=SCoVaR ~"|"~sum(X[i])<="VaR("~sum(X[i])~"):"), pos = pos, cex = 1, col = "darkgreen")
points(t_CoVaR_Sys_exc[[3]], rep(at[8], t_CoVaR_Sys_exc[[2]]), pch = 4, col = "darkgreen")

#MCoVaR condition
text(coord, at[9], expression(symbol("\042")~i:~ X[i]<="VaR("~X[i]~"):"), pos = pos, cex = 1)
points(Multi_exc, rep(at[10], length(Multi_exc)), pch = 4)

#MCoVaR exceedences
text(coord, at[11], expression(BTC<=MCoVaR ~"|"~symbol("\042")~i:~ X[i]<="VaR("~X[i]~"):"), pos = pos, cex = 1, col = "skyblue4")
points(t_CoVaR_Multi_exc[[3]], rep(at[12], t_CoVaR_Multi_exc[[2]]), pch = 4, col = "skyblue4")

#legend
legend("bottom", horiz = TRUE, legend = c("log-return of BTC", "5% VaR of BTC", "5% VCoVaR", "5% SCoVaR", "5% MCoVaR"), 
       fill = c("blue3", "red3", "green3", "darkgreen", "skyblue4"), bty = "n")
